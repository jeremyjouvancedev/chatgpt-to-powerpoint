...
def generate_langchain_chain(model_type='ChatOpenAI', model_name='gpt-3.5-turbo', nb_of_slides=10):
    if model_type == 'ChatOpenAI':
        chat_model = ChatOpenAI(model_name=model_name, verbose=True)
...
def generate_slides(nb_of_slides, topic, output, model_type, model_name, load_json = False):
    response = None

    if not load_json:
        chain = generate_langchain_chain(model_type, model_name, nb_of_slides)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelType', type=str, default='ChatOpenAI', help='Model type to use for generating the presentation. Default is ChatOpenAI.')
    parser.add_argument('--model', type=str, default='gpt-3.5-turbo', help='Model to use for generating the presentation. Default is gpt-3.5-turbo.')
    ...
    generate_slides(args.slides, args.topic, args.output, args.modelType, args.model)